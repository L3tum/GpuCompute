...
{
    internal class SwapChainApplication<T> : Win32Application
        where T : struct, IComputeShader
    {
        ...
        /// </summary>
        private readonly ComPtr<ID3D12CommandQueue> d3D12CommandQueue;

        /// <summary>
        ///     The <see cref="ID3D12Device" /> pointer for the device currently in use.
        /// </summary>
        private readonly ComPtr<ID3D12Device> d3D12Device;

        /// <summary>
        ///     The <see cref="ID3D12Fence" /> instance used for graphics operations.
        /// </summary>
        private readonly ComPtr<ID3D12Fence> d3D12Fence;
        ...
        /// </summary>
        private readonly ComPtr<IDXGISwapChain1> dxgiSwapChain1;
        ...
        public override unsafe void OnInitialize(HWND hwnd)
        {
            ...
            fixed (IDXGISwapChain1** dxgiSwapChain1 = this.dxgiSwapChain1)
            {
                ...
                _ = FX.CreateDXGIFactory2(FX.DXGI_CREATE_FACTORY_DEBUG, FX.__uuidof<IDXGIFactory2>(),
                    (void**) dxgiFactory2.GetAddressOf());
                ...
                dxgiSwapChainDesc1.SampleDesc = new DXGI_SAMPLE_DESC(1, 0);
        ...
        private unsafe void ApplyResize()
        {
            ...
            var d3D12Resource0Description = this.d3D12Resource0.Get()->GetDesc();
        ...
        public override unsafe void OnUpdate(TimeSpan time)
        {
            ...
            Gpu.Default.For(texture!.Width, texture.Height, GetComputeShader(texture, time));

            using ComPtr<ID3D12Resource> d3D12Resource = default;

            // Get the underlying ID3D12Resource pointer for the texture
            _ = InteropServices.TryGetID3D12Resource(texture, FX.__uuidof<ID3D12Resource>(),
                (void**) d3D12Resource.GetAddressOf());
            ...
            d3D12CommandQueue.Get()->ExecuteCommandLists(1,
                (ID3D12CommandList**) d3D12GraphicsCommandList.GetAddressOf());
            d3D12CommandQueue.Get()->Signal(d3D12Fence.Get(), nextD3D12FenceValue);
        ...
        protected virtual T GetComputeShader(IReadWriteTexture2D<Float4> tex, TimeSpan time)
        {

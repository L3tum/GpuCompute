    ...
    internal static unsafe class Win32ApplicationRunner
    {
        ...
        private static bool CreateWindow()
        {
            ...
            fixed (char* windowTitle = application.GetType().ToString())
            {
                ...
                _ = FX.AdjustWindowRect((RECT*) &windowRect, FX.WS_OVERLAPPEDWINDOW, FX.TRUE);
        ...
        public static int Run(Win32Application application)
        {
            ...
            {
                var (renderApplication, token) = ((Win32Application, CancellationToken)) args!;
                ...
                while (!token.IsCancellationRequested)
                {
                    if (frameStopwatch.ElapsedTicks >= targetFrameTimeInTicksFor60fps)
                    {
                        ...
                        renderApplication.OnUpdate(startStopwatch.Elapsed);
                    }
                }
            });
        ...
        private static nint WindowProc(IntPtr hwnd, uint uMsg, nuint wParam, nint lParam)
        {
            switch (uMsg)
            {
                ...
                case FX.WM_KEYDOWN:
                {
                    ...
                    else if ((ConsoleKey) wParam == ConsoleKey.Backspace)
                    {
                        FX.DestroyWindow(hwnd);
                    }
